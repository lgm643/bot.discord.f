...
CORRECTIONS v2 :
  [1] BUG monthly_msgs / monthly_voice supprimé.
      L'ancienne version multipliait weekly × 4 et affichait ce chiffre comme
      réel. C'est une approximation grossière (et fausse pour les semaines
      incomplètes). Ces champs sont maintenant retournés à None et affichés
      honnêtement comme « N/D » dans la commande !statsserveur.

  [2] compute_server_stats() : les deux boucles guild.members (arrivées + stats)
      sont fusionnées en une seule passe pour réduire la charge CPU.

  [3] _handle_motd_role (appelée depuis weekly.py) : utilise asyncio.gather()
      pour retire/attribuer les rôles en parallèle au lieu de la boucle série,
      évitant le blocage lors du reset hebdo sur de grands serveurs.
...
def compute_server_stats(guild: discord.Guild) -> dict:
    ...
    [2] Passe unique sur guild.members : arrivées + stats en un seul for-loop.
        monthly_msgs et monthly_voice sont None (non disponibles honnêtement).
    """
    ...
    top_daily_msg  : tuple[int, int]   = (0, 0)
    top_daily_xp   : tuple[int, int]   = (0, 0)
    top_daily_voice: tuple[int, float] = (0, 0.0)

    # [2] Passe unique : arrivées + stats agrégées
    for m in guild.members:
        ...
        if u:
            ...
            if dm > top_daily_msg[1]:
                top_daily_msg = (m.id, dm)
            if dxp > top_daily_xp[1]:
                top_daily_xp = (m.id, dxp)
            if dv > top_daily_voice[1]:
                top_daily_voice = (m.id, dv)
    ...
    return {
        ...
        "monthly_msgs":    None,   # [1] Non disponible — ancienne valeur ×4 supprimée
        "daily_voice":     daily_voice,
        "weekly_voice":    weekly_voice,
        "monthly_voice":   None,   # [1] Non disponible — ancienne valeur ×4 supprimée
...
def compute_motd_messages(guild: discord.Guild, cfg: dict) -> int | None:
    """Membre de la semaine catégorie Messages."""
...
def compute_motd_vocal(guild: discord.Guild, cfg: dict) -> int | None:
    """Membre de la semaine catégorie Vocal."""
    if not cfg.get("motd_enabled", True):
        return None
    data = load_user_data(guild.id)
