...
def _help_embed_accueil(is_staff_user: bool) -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            ("\n🔨 Modération · 📊 Stats & Hebdo · ⚙️ Configuration" if is_staff_user else "")
        ),
        color=0x9B59B6
    )
...
def _help_embed_general() -> discord.Embed:
    ...
    embed.add_field(
        name="📊 `!statsserveur` · alias : `!stats` `!statistiques`",
        value="Affiche les statistiques complètes du serveur : membres, messages, vocal et tops du jour.",
        inline=False
    )
    embed.add_field(
...
def _help_embed_protections() -> discord.Embed:
    ...
    embed.add_field(
        ...
        value=(
            ...
            "2. **Mute automatique 5 minutes** si ça recommence\n"
            "*(configurable en expulsion via `!config` → ⚙️ Sécurité → `spam_action`)*\n"
...
def _help_embed_stats() -> discord.Embed:
    embed = discord.Embed(title="📊 Stats & Hebdo 🔒", color=0x9B59B6)
    embed.add_field(
        name="📊 `!statsserveur` · alias : `!stats` `!statistiques`",
        value=(
            "Affiche les statistiques complètes du serveur :\n"
            "• 👥 Membres (total, en ligne, arrivées du jour / semaine)\n"
            "• 💬 Messages (aujourd'hui / semaine / mois)\n"
            "• 🎙️ Vocal (aujourd'hui / semaine / mois)\n"
            "• 🏆 Tops du jour (messages, XP, vocal)\n"
            "Accessible à **tous les membres**."
        ),
        inline=False
    )
    embed.add_field(
        name="📅 `!hebdo` · alias : `!classementsemaine` `!weekly` 🔒",
        value=(
            "Affiche le classement hebdomadaire **en temps réel** sans attendre le lundi.\n"
            "Contenu :\n"
            ...
            "⚠️ N'envoie pas le classement automatique ni ne remet les stats à zéro."
        ),
        inline=False
    )
    embed.add_field(
        ...
        value=(
            "Chaque **lundi à 00h01 UTC**, le bot envoie automatiquement le classement\n"
            "dans le salon configuré (`!config` → 📊 Stats & Hebdo → `salon_hebdo`).\n"
            "Les stats sont remises à zéro après envoi.\n\n"
            "**Membres de la semaine (2 catégories) :**\n"
            "💬 Celui avec le plus de messages\n"
            "🎙️ Celui avec le plus de temps vocal\n"
            "Rôles configurables : `role_motd_msg` et `role_motd_vocal`"
        ),
        inline=False
    )
    embed.set_footer(text="🔒 = Staff uniquement (sauf !statsserveur)")
    return embed
...
def _help_embed_moderation() -> discord.Embed:
    ...
    embed.add_field(
        name="📊 `!hebdo` · alias : `!classementsemaine` `!weekly` 🔒",
        value="Affiche le classement hebdomadaire en temps réel sans attendre le lundi.",
        inline=False
    )
    embed.add_field(
        name="📣 `!say #salon message` · alias : `!dit`",
