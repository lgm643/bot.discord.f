...
async def giveaway_cmd(ctx, duree: str = None, *, reward: str = None):
    ...
    # Persister l'état pour survie au redémarrage
    from bot.events.restore import _save_active_giveaways
    _save_active_giveaways()
...
async def _end_giveaway(gw_id, delay, channel, reward):
    ...
    if not gw:
        return
    # Mettre à jour la persistance (giveaway terminé)
    from bot.events.restore import _save_active_giveaways
    _save_active_giveaways()
