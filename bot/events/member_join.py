...
async def on_member_join(member: discord.Member):
    ...
    if welcome_channel:
        try:
            age_days_acc = (datetime.now(timezone.utc) - member.created_at).days
            bvn_embed = discord.Embed(
                title=f"👋 Bienvenue sur {member.guild.name} !",
                description=(
                    f"Salut {member.mention} ! Heureux de t'accueillir parmi nous 🔥\n\n"
                    f"📋 Consulte nos salons d'information pour débuter.\n"
                    f"🎫 Tu as une question ? Utilise `!ticket` pour contacter le staff."
                ),
                color=0x2ECC71,
                timestamp=now_utc(),
            )
            bvn_embed.set_thumbnail(url=member.display_avatar.url)
            bvn_embed.set_footer(
                text=f"Compte créé il y a {age_days_acc} jour(s) • {member.guild.member_count} membres"
            )
            await welcome_channel.send(embed=bvn_embed)
