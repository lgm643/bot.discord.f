    ...
    if not is_staff(member):
        ...
        if len(spam_tracker[gid][uid]) > spam_limit:
            if uid in spam_warned[gid]:
                ...
                cfg_spam = load_config(message.guild.id)
                spam_action = cfg_spam.get("spam_action", "warn")
                if spam_action == "kick":
                    try:
                    ...
                    # Action par défaut : mute temporaire 5 minutes
                    try:
                        mute_role = discord.utils.get(message.guild.roles, name="Muted")
                        if mute_role:
                            await member.add_roles(mute_role, reason="Anti-spam automatique (mute 5 min)")
                            await message.channel.send(
                                f"🔇 {member.mention} muté **5 minutes** pour spam excessif.",
                                delete_after=10,
                            )
                            embed = discord.Embed(title="🔇 Mute Anti-Spam", color=0xE67E22, timestamp=now_utc())
                            embed.add_field(name="👤 Membre", value=f"{member} ({member.id})", inline=True)
                            embed.add_field(name="⏱️ Durée", value="5 minutes", inline=True)
                            await send_log(message.guild, embed)
                            from bot.commands.moderation import _schedule_unmute
                            import asyncio as _aio
                            _aio.create_task(_schedule_unmute(message.guild, member, mute_role, 300, "5 minutes"))
                    except discord.Forbidden:
                        pass
            else:
...
async def on_message_delete(message: discord.Message):
    if not message.guild:
        return
    # Ignorer les messages du bot (auto-delete, delete_after, anti-spam, etc.)
    if message.author.bot:
        return
    # Ignorer les messages sans contenu réel (embeds seuls envoyés par le bot)
    if not message.content and not message.attachments:
        return
    # Ignorer les suppressions automatiques dans les salons catalogue/commandes
    ...
    if (cat_ch and message.channel.id == cat_ch.id) or (cmd_ch and message.channel.id == cmd_ch.id):
        return
